...
SERVICE_ACCOUNT_IAM_ROLE=EKS-AMG-ServiceAccount-Role
SERVICE_ACCOUNT_IAM_ROLE_DESCRIPTION="IAM role to be used by Managed Grafana to link to a AMP DataSource"
SERVICE_ACCOUNT_IAM_POLICY=AWSManagedGrafanaDataSourcesPolicy
...
#
OLD_TRUST_RELATIONSHIP=$(aws iam get-role --role-name $SERVICE_ACCOUNT_IAM_ROLE --query 'Role.AssumeRolePolicyDocument.Statement[]' --output json 2>/dev/null)
...
EOF
echo "${TRUST_POLICY}" > AMGTrustPolicy.json
#
...
EOF
echo "${PERMISSION_POLICY}" > AMGPermissionPolicy.json

#
# Create an IAM permission policy to be associated with the role, if the policy does not already exist
#
SERVICE_ACCOUNT_IAM_POLICY_ID=$(aws iam get-policy --policy-arn $SERVICE_ACCOUNT_IAM_POLICY_ARN --query 'Policy.PolicyId' --output text 2>/dev/null || echo "PolicyNotFound")
if [ "$SERVICE_ACCOUNT_IAM_POLICY_ID" = "PolicyNotFound" ];
then
  echo "Creating a new permission policy $SERVICE_ACCOUNT_IAM_POLICY"
  aws iam create-policy --policy-name $SERVICE_ACCOUNT_IAM_POLICY --policy-document file://AMGPermissionPolicy.json
else
...
#
SERVICE_ACCOUNT_IAM_ROLE_ARN=$(aws iam get-role --role-name $SERVICE_ACCOUNT_IAM_ROLE --query 'Role.Arn' --output text 2>/dev/null || echo "RoleNotFound")
if [ "$SERVICE_ACCOUNT_IAM_ROLE_ARN" = "RoleNotFound" ];
then
  ...
  --assume-role-policy-document file://AMGTrustPolicy.json \
  ...
  #
  aws iam update-assume-role-policy --role-name $SERVICE_ACCOUNT_IAM_ROLE --policy-document file://AMGTrustPolicy.json
fi
