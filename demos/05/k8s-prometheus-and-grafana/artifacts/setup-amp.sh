...
EOF
echo "${TRUST_POLICY}" > AMPTrustPolicy.json
#
...
EOF
echo "${PERMISSION_POLICY}" > AMPPermissionPolicy.json
...
then
  ...
  aws iam create-policy --policy-name $SERVICE_ACCOUNT_IAM_POLICY --policy-document file://AMPPermissionPolicy.json
else
...
then
  ...
  --assume-role-policy-document file://AMPTrustPolicy.json \
  ...
  #
  aws iam update-assume-role-policy --role-name $SERVICE_ACCOUNT_IAM_ROLE --policy-document file://AMPTrustPolicy.json
fi
